<template>
  <div>
    <h2 id="page-heading" data-cy="TipoReciboHeading">
      <span v-text="$t('proyecto03App.tipoRecibo.home.title')" id="tipo-recibo-heading">Tipo Recibos</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('proyecto03App.tipoRecibo.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TipoReciboCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('proyecto03App.tipoRecibo.home.createLabel')"> Create a new Tipo Recibo </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tipoRecibos && tipoRecibos.length === 0">
      <span v-text="$t('proyecto03App.tipoRecibo.home.notFound')">No tipoRecibos found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="tipoRecibos">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('fecha')">
              <span v-text="$t('proyecto03App.tipoRecibo.fecha')">Fecha</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'fecha'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('tipoRecibo')">
              <span v-text="$t('proyecto03App.tipoRecibo.tipoRecibo')">Tipo Recibo</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'tipoRecibo'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('clave')">
              <span v-text="$t('proyecto03App.tipoRecibo.clave')">Clave</span>
            ...
            <th scope="row" v-on:click="changeOrder('estatus')">
              <span v-text="$t('proyecto03App.tipoRecibo.estatus')">Estatus</span>
            ...
            <th scope="row" v-on:click="changeOrder('usuario')">
              <span v-text="$t('proyecto03App.tipoRecibo.usuario')">Usuario</span>
            ...
            <th scope="row" v-on:click="changeOrder('fechaMod')">
              <span v-text="$t('proyecto03App.tipoRecibo.fechaMod')">Fecha Mod</span>
        ...
        <tbody>
          <tr v-for="tipoRecibo in tipoRecibos" :key="tipoRecibo.id" data-cy="entityTable">
            ...
            <td v-text="$t('proyecto03App.Estatus.' + tipoRecibo.estatus)">{{ tipoRecibo.estatus }}</td>
            <td>{{ tipoRecibo.usuario }}</td>
            <td>{{ tipoRecibo.fechaMod }}</td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="proyecto03App.tipoRecibo.delete.question" data-cy="tipoReciboDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-tipoRecibo-heading" v-text="$t('proyecto03App.tipoRecibo.delete.question', { id: removeId })">
    ...
    <div v-show="tipoRecibos && tipoRecibos.length > 0">
