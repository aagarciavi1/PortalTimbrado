...
public interface ParametrosRepository extends ReactiveCrudRepository<Parametros, Long>, ParametrosRepositoryInternal {
    ...
    @Query("SELECT * FROM parametros entity WHERE entity.registinstit_id = :id")
    Flux<Parametros> findByRegistinstit(Long id);

    @Query("SELECT * FROM parametros entity WHERE entity.registinstit_id IS NULL")
    Flux<Parametros> findAllWhereRegistinstitIsNull();

    @Query("SELECT * FROM parametros entity WHERE entity.tipocfdi_id = :id")
    Flux<Parametros> findByTipocfdi(Long id);

    @Query("SELECT * FROM parametros entity WHERE entity.tipocfdi_id IS NULL")
    Flux<Parametros> findAllWhereTipocfdiIsNull();
